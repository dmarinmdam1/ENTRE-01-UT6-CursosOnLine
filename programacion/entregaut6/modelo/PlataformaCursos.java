...
public class PlataformaCursos
{
    ...
    public void addCurso(String categoria, Curso curso)
    {
        ...
        {
            ArrayList<Curso> nuevaListaCursos = new ArrayList<>();
            nuevaListaCursos.add(curso);
            plataforma.put(categoria, nuevaListaCursos);
        }
    }
    ...
    {
        String[] datosSeparados = lineaCurso.split(SEPARADOR);

        // ----- //

        String nombreCurso = datosSeparados[0];

        // ----- //

        String fechaEnTexto = datosSeparados[1].trim();
        String[] fechaSeparada = fechaEnTexto.split("/");

        LocalDate fechaCurso = LocalDate.of(Integer.valueOf(fechaSeparada[2]), Integer.valueOf(fechaSeparada[1]), Integer.valueOf(fechaSeparada[0]));

        // ----- //

        Nivel nivelCurso = Nivel.valueOf(datosSeparados[2].toUpperCase().trim());

        // ----- //

        return new Curso(nombreCurso, fechaCurso, nivelCurso);
    }
    ...
    public TreeSet<String> obtenerCategorias()
    {
        ...
        retorno.addAll(plataforma.keySet());
    ...
    public TreeSet<String> borrarCursosDe(String categoria, Nivel nivel)
    {
        ...
        ArrayList<Curso> cursosDeLaCategoria = plataforma.get(categoria.toUpperCase());
        Iterator<Curso> it = cursosDeLaCategoria.iterator();

        while(it.hasNext())
        {
            Curso cursoActual = it.next();

            if(nivel.equals(cursoActual.getNivel()))
            {
                cursosBorrados.add(cursoActual.getNombre());
                it.remove();
            }
        }
        ...
        while(it.hasNext())
        {
            ...
            for(Curso curso: cursos)
            {
                if(curso.getFecha().isBefore(fechaMasAntigua))
                {
                    ...
                    cursoMasAntiguo = curso.getNombre();
                }
            }
        }
        
        return cursoMasAntiguo;
    }
